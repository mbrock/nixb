    ...
    for (auto x = 0 * ch; x < view.width (); ++x)
      {
        ...
        expect (cell->bg == bg)
            << "bg at x=" << x.numerical_value_in (ch) << " (" << cell->bg
            << " == " << bg << ")";
        if (cell->glyph != ' ')
          has_bar_glyph = true;
      }
...
suite diff_tests = [] {
  "diff_rasters emits nothing for identical rasters"_test = [] {
    ...
    diff_rasters (front, back, [&] (const ChangeRun &r) { runs.push_back (r); });
  ...
  "diff_rasters emits one run for single changed cell"_test = [] {
    ...
    expect (runs[0].origin == Pos::at (1 * ch, 0 * ln)) << "correct position";
  ...
  "diff_rasters batches consecutive changed cells with same colors"_test = [] {
    ...
    expect (runs.size () == 1_ul) << "consecutive changes batched into one run";
    expect (runs[0].origin == Pos::at (2 * ch, 0 * ln)) << "starts at first change";
  ...
  "diff_rasters tracks color state across runs"_test = [] {
    ...
    expect (runs[0].fg_change == red) << "first run sets red";
    expect (!runs[1].fg_change.has_value ()) << "second run omits redundant color";
  };
