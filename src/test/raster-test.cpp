...
namespace nxb::test
{
  ...
  template <typename Layout> struct RenderChecker
  {
    const Layout &layout;
    std::vector<std::string> expected;

    RenderChecker (const Layout &l) : layout (l) {}
    RenderChecker (RenderChecker &&) = default;
    RenderChecker &operator= (RenderChecker &&) = default;

    RenderChecker &
    operator| (std::string_view row)
    {
    ...
    ~RenderChecker ()
    {
      ...
      for (std::size_t row_idx = 0; row_idx < expected.size (); ++row_idx)
        {
          std::string actual;
  ...
  RenderChecker<Layout>
  renders (const Layout &layout)
  {
      ...
      "terminal to ANSI coords"_test = []
        {
          ...
          auto to_ansi_row = [] (int t)
            {
              return (to_ansi (terminal_origin_v + t * ln)
                      - ansi_origin_v)
