...
// MSVC STL and libstdc++ provide __cpp_lib_format.
    ...
    or (defined(_LIBCPP_VERSION) and _LIBCPP_VERSION >= 170000)
...
#if defined(__has_builtin) and defined(__GNUC__) and (__GNUC__ < 10)          \
    and not defined(__clang__)
#undef __has_builtin
#endif
...
private:
  ...
  invoke_impl (void *data, TArgs... args) -> R
  {
...
is_match (std::string_view input, std::string_view pattern) -> bool
{
...
[[nodiscard]] constexpr auto
match (const TPattern &pattern, const TStr &str) -> std::vector<TStr>
{
...
[[nodiscard]] inline auto
split (T input, TDelim delim) -> std::vector<T>
{
  std::vector<T> output{};
  std::size_t first{};
...
constexpr auto
regex_match (const char *str, const char *pattern) -> bool
{
  if (*pattern == '\0' && *str == '\0')
    {
      return true;
...
[[nodiscard]] constexpr auto
get_template_function_name_use_type () -> std::string_view
{
...
template <typename TargetType>
[[nodiscard]] constexpr auto
get_template_function_name_use_decay_type () -> std::string_view
{
...
template <typename TargetType>
[[nodiscard]] constexpr auto
decay_type_name () -> std::string_view
{
  const std::string_view raw_type_name
...
[[nodiscard]] constexpr auto
abs_diff (const T t, const U u) -> decltype (t < u ? u - t : t - u)
{
  return t < u ? u - t : t - u;
}
...
template <typename CharT, std::size_t SIZE> struct fixed_string
{
...
private:
  ...
  template <class T>
  static constexpr auto
  run_impl (T test, const TArg &arg) -> decltype (test (arg), void ())
  {
    test (arg);
  }
...
struct cfg
{
  ...
  static std::optional<cfg::option>
  find_arg (std::string_view arg)
  {
    ...
    for (const auto &[cmd, arg, val, description] : cfg::options)
      {
        ...
        const auto pad_by
            = (s.size () <= opt_width) ? opt_width - s.size () : 0;
  ...
  static void
  parse_arg_with_fallback (int argc, const char *argv[])
  {
        ...
        if (std::holds_alternative<std::reference_wrapper<std::size_t>> (var))
          {
            ...
            auto val
                = static_cast<std::size_t> (std::stoull (argument, &last));
        ...
        if (std::holds_alternative<std::reference_wrapper<std::string>> (var))
          {
            ...
            std::get<std::reference_wrapper<std::string>> (var).get ()
                = argv[i];
            continue;
          }
      }
...
template <std::floating_point T> struct value<T> : op
{
  ...
  constexpr /*explicit(false)*/ value (const T &val)
      : value{ val, T (1)
                        / math::pow (
                            T (10), math::den_size<unsigned long long> (val)) }
  {
  }
  [[nodiscard]] constexpr explicit
  operator T () const
...
template <class TLhs, class TRhs> struct eq_ : op
{
  constexpr eq_ (const TLhs &lhs = {}, const TRhs &rhs = {})
      : lhs_{ lhs }, rhs_{ rhs }, value_{ [&] {
          ...
          if constexpr (type_traits::has_static_member_object_value_v<TLhs>
                        and type_traits::has_static_member_object_value_v<
                            TRhs>)
            {
              return lhs.value == rhs.value;
                                 ...
                                 TLhs>
                             and type_traits::
                                 has_static_member_object_epsilon_v<TRhs>)
            {
...
template <class TLhs, class TRhs, class TEpsilon> struct approx_ : op
{
  constexpr approx_ (const TLhs &lhs = {}, const TRhs &rhs = {},
      ...
      : lhs_{ lhs }, rhs_{ rhs }, epsilon_{ epsilon }, value_{ [&] {
          ...
          if constexpr (type_traits::has_static_member_object_value_v<TLhs>
                        and type_traits::has_static_member_object_value_v<TRhs>
                        and type_traits::has_static_member_object_value_v<
                            TEpsilon>)
            {
              return math::abs_diff (TLhs::value, TRhs::value)
                     < TEpsilon::value;
            }
          else
            {
  ...
  constexpr lt_ (const TLhs &lhs = {}, const TRhs &rhs = {})
      : lhs_{ lhs }, rhs_{ rhs }, value_{ [&] {
          ...
          if constexpr (type_traits::has_static_member_object_value_v<TLhs>
            ...
            {
#if defined(_MSC_VER) && !defined(__clang__)
              // for some reason, accessing the static member via :: does not
              // compile on MSVC, and the next line does not compile on clang,
              // so we have to use the ifdef here
              return lhs.value < rhs.value;
#else
...
public:
  ...
  auto &
  operator<< (const detail::approx_<TLhs, TRhs, TEpsilon> &op)
  {
    return (*this << color (op) << op.lhs () << " ~ (" << op.rhs () << " +/- "
  ...
  template <class TLhs, class TRhs>
  auto &
  operator<< (const detail::neq_<TLhs, TRhs> &op)
  {
    return (*this << color (op) << op.lhs () << " != " << op.rhs ()
                  << colors_.none);
  }
  ...
  template <class TExpr>
  auto &
  operator<< (const detail::aborts_<TExpr> &op)
  {
...
public:
  ...
  auto
  on (events::run_begin) -> void
  {
  }

  auto
  on (events::test_begin test_begin) -> void
  {
  ...
  auto
  on (events::test_run test_run) -> void
  {
    printer_ << "\n \"" << test_run.name << "\"...";
  }

  auto
  on (events::test_skip test_skip) -> void
  {
  ...
  auto
  on (events::test_end) -> void
  {
    if (asserts_.fail > fails_)
      {
        ++tests_.fail;
        printer_ << '\n'
                 << printer_.colors ().fail << "FAILED"
                 << printer_.colors ().none << '\n';
      }
    else
      {
        ++tests_.pass;
        printer_ << printer_.colors ().pass << "PASSED"
                 << printer_.colors ().none << '\n';
      }
  }
  ...
  template <class TMsg>
  auto
  on (events::log<TMsg> l) -> void
  {
    printer_ << l.msg;
  }

  auto
  on (events::exception exception) -> void
  {
  ...
  template <class TExpr>
  auto
  on (events::assertion_pass<TExpr>) -> void
  {
    ++asserts_.pass;
  }
  ...
  template <class TExpr>
  auto
  on (events::assertion_fail<TExpr> assertion) -> void
  {
  ...
  auto
  on (const events::fatal_assertion &) -> void
  {
  }

  auto
  on (events::summary) -> void
  {
...
template <class TPrinter = printer> class reporter_junit
{
  template <typename Key, typename T> using map = std::unordered_map<Key, T>;
  ...
  void
  check_for_scope (std::string_view test_name)
  {
  ...
  void
  pop_scope (std::string_view test_name_sv)
  {
...
public:
  ...
  auto
  on (events::suite_begin suite) -> void
  {
  ...
  auto
  on (events::test_begin test_event) -> void
  ...
  auto
  on (events::test_end test_event) -> void
  {
  ...
  auto
  on (events::test_run test_event) -> void
  { // starts nested test
    on (events::test_begin{ .type = test_event.type,
                            .name = test_event.name });
  }

  auto
  on (events::test_finish test_event) -> void
  { // finishes nested test
    on (events::test_end{ .type = test_event.type, .name = test_event.name });
  }

  auto
  on (events::test_skip test_event) -> void
  {
    ss_out_.clear ();
  ...
  on (events::exception exception) -> void
  {
    ...
    if (detail::cfg::abort_early
      ...
      {
        std::cerr << "early abort for test : " << active_test_.top ()
                  << "after ";
...
protected:
  void
  print_duration (auto &printer) const noexcept
  {
  ...
  void
  print_console_summary (std::ostream &out_stream, std::ostream &err_stream)
  {
  ...
  void
  print_junit_summary (std::ostream &stream)
  {
    // aggregate results
    size_t n_tests = 0;
    size_t n_fails = 0;
  ...
  void
  print_result (std::ostream &stream, const std::string &suite_name,
...
class runner
{
  ...
  public:
    ...
    constexpr auto
    operator() (const std::size_t level, const TPath &path) const -> bool
    {
...
public:
  ...
  template <class... Ts>
  auto
  on (events::test<Ts...> test)
  {
    path_[level_] = test.name;

    if (detail::cfg::list_tags)
      {
        std::for_each (test.tag.cbegin (), test.tag.cend (),
                       [] (const auto &tag) {
    ...
    if (!detail::cfg::query_pattern.empty ())
      {
        ...
        bool matches
            = utility::regex_match (test.name.data (), regex.c_str ());
    ...
    if (filter_ (level_, path_))
      {
        if (not level_++)
          {
            reporter_.on (events::test_begin{ .type = test.type,
                                              .name = test.name,
          ...
          {
            reporter_.on (
                events::test_run{ .type = test.type, .name = test.name });
          }
        ...
        if (not --level_)
          {
            reporter_.on (
                events::test_end{ .type = test.type, .name = test.name });
          }
        else
  ...
  auto
  on (events::fatal_assertion fatal_assertion)
  {
    ...
    for (const auto &[suite, suite_name] : suites_)
      {
        ...
        if constexpr (requires { reporter_.on (events::suite_begin{}); })
          {
            reporter_.on (
                events::suite_begin{ .type = "suite", .name = suite_name });
          }
        suite ();
        if constexpr (requires { reporter_.on (events::suite_end{}); })
          {
            reporter_.on (
                events::suite_end{ .type = "suite", .name = suite_name });
          }
      }
    suites_.clear ();
...
struct test
{
  ...
  template <class... Ts>
  constexpr auto
  operator= (test_location<void (*) ()> _test)
  {
  ...
  constexpr void
  operator= (void (*_test) (std::string_view, std::string_view)) const
  {
    _test (type, name);
  }
...
struct log
{
  ...
  {
    on<std::string> (events::log{
        std::vformat (fmt.get (), std::make_format_args (args...)) });
  }
#else
  template <class... Args>
  void
  operator() (std::string_view fmt, Args &&...args)
  {
    on<std::string> (
...
template <class TExpr> struct fatal_ : op
{
  ...
  operator bool () const
  {
    if (static_cast<bool> (expr_))
      ...
      {
        cfg::wip = true;
        void (on<TExpr> (events::assertion<TExpr>{
            .expr = expr_, .location = cfg::location }));
    ...
    if (not value_)
      {
        on<T> (events::log{ ' ' });
        if constexpr (requires {
                        requires std::invocable<TMsg>
                                     and not std::is_void_v<
...
{
[[nodiscard]] inline auto
operator""_test (const char *name, std::size_t size)
{
...
template <class TArg>
inline std::string
format_test_parameter ([[maybe_unused]] const TArg &arg, const int counter)
{
  return std::to_string (counter) + get_ordinal_suffix (counter)
         + " parameter";
}
...
inline std::string
format_test_parameter (const bool &arg, [[maybe_unused]] const int counter)
{
    ...
    apply (
        [=, &counter] (const auto &...args) {
          (detail::on<F> (events::test<F, Ts>{
               .type = type,
               .name
               = unique_name.template operator()<Ts> (name, args, counter),
               .tag = {},
               .location = {},
               .arg = args,
               .run = f }),
...
template <class T>
constexpr auto
operator% (const T &t, const placeholder_gcc_t &)
{
...
constexpr auto
expect (const TExpr &expr, const reflection::source_location &sl
                           = reflection::source_location::current ())
{
...
template <fixed_string suite_name = "unnamed suite"> struct suite
{
  reflection::source_location location{};
...
[[maybe_unused]] constexpr auto test
    = [] (const auto name) { return detail::test{ "test", name }; };
[[maybe_unused]] constexpr auto should = test;
[[maybe_unused]] inline auto tag
    = [] (const auto name) { return detail::tag{ { name } }; };
...
template <class TLhs, class TRhs>
  requires type_traits::is_stream_insertable_v<TLhs>
           && type_traits::is_stream_insertable_v<TRhs>
[[nodiscard]] constexpr auto
neq (const TLhs &lhs, const TRhs &rhs)
{
  return detail::neq_{ lhs, rhs };
}
template <class TLhs, class TRhs>
...
namespace bdd
{
[[maybe_unused]] constexpr auto feature
    = [] (const auto name) { return detail::test{ "feature", name }; };
[[maybe_unused]] constexpr auto scenario
    = [] (const auto name) { return detail::test{ "scenario", name }; };
[[maybe_unused]] constexpr auto given
    = [] (const auto name) { return detail::test{ "given", name }; };
[[maybe_unused]] constexpr auto when
    = [] (const auto name) { return detail::test{ "when", name }; };
[[maybe_unused]] constexpr auto then
    = [] (const auto name) { return detail::test{ "then", name }; };
...
class steps
{
  ...
  public:
    ...
    template <class TExpr>
    auto
    operator= (const TExpr &expr) -> void
    {
...
public:
  ...
  auto
  operator| (const TGherkin &gherkin)
  {
  ...
  auto
  feature (const std::string &pattern)
  {
    return step{ *this, "Feature: " + pattern };
  }
  auto
  scenario (const std::string &pattern)
  {
    return step{ *this, "Scenario: " + pattern };
  }
  auto
  given (const std::string &pattern)
  {
    return step{ *this, "Given " + pattern };
  }
  auto
  when (const std::string &pattern)
  {
    return step{ *this, "When " + pattern };
  }
  auto
  then (const std::string &pattern)
  {
...
private:
  template <class TPattern>
  auto
  next (const TPattern &pattern) -> void
  {
    const auto is_scenario = [&pattern] (const auto &_step) {
      ...
      return pattern.find (scenario) == std::string::npos
             and _step.find (scenario) != std::string::npos;
    };

    const auto call_steps
        = [this, is_scenario] (const auto &_step, const auto i) {
            for (const auto &[name, call] : call_steps_)
              {
                ...
                if (utility::is_match (_step, name)
                    or not std::empty (utility::match (name, _step)))
                  {
                    step_ = i;
                    call (_step);
                  }
...
namespace spec
{
[[maybe_unused]] constexpr auto describe
    = [] (const auto name) { return detail::test{ "describe", name }; };
[[maybe_unused]] constexpr auto it
    = [] (const auto name) { return detail::test{ "it", name }; };
} // namespace spec
...
__attribute__ ((constructor (101))) inline void
cmd_line_args (int argc, const char *argv[])
{
