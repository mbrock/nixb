...
namespace nxb::test
{
  ...
  std::string
  render_to_string (ui::TerminalCompositor &compositor,
                    const auto &layout, Size size)
  {
    ...
    for (std::size_t i = 0; i < expected.size (); ++i)
      {
        ...
        while (!actual.empty () && actual.back () == ' ')
          actual.pop_back ();
        expect (actual == expected[i]) << fmt::format (
            "row {}: '{}' != '{}'", i, actual, expected[i]);
      }
  }
      ...
      "renders text at correct position"_test = []
        {
          ...
          auto output = render_to_string (compositor, tui::text ("Hello"),
                                          { 20 * ch, 5 * ln });
          term.write (output);
      ...
      "renders column layout"_test = []
        {
          ...
          auto layout = tui::column (tui::text ("One"), tui::text ("Two"),
                                     tui::text ("Three"));
          term.write (
      ...
      "renders styled text with colors"_test = []
        {
          ...
          term.write (render_to_string (compositor,
      ...
      "HUD appears at bottom of terminal"_test = []
        {
          ...
          auto layout = tui::column (tui::text ("HUD-LINE-1"),
                                     tui::text ("HUD-LINE-2"));
          term.write (
              render_to_string (compositor, layout, { 20 * ch, 2 * ln }));
      ...
      "full screen mode when HUD equals terminal height"_test = []
        {
          ...
          compositor.set_hud_height (3 * ln, 3 * ln);

          auto layout = tui::column (tui::text ("A"), tui::text ("B"),
                                     tui::text ("C"));
          term.write (
              render_to_string (compositor, layout, { 20 * ch, 3 * ln }));
  ...
  suite scroll_region_tests = []
      ...
      "println scrolls content without affecting HUD"_test = []
        {
          ...
          // Render HUD
          auto hud = tui::column (tui::text ("HUD-LINE-1"),
                                  tui::text ("HUD-LINE-2"));
          term.write (
    ...
    check_display (term, {
      ...
      "HUD-LINE-2",  // row 5
          ...
          // Third log line - now scroll region fills
