...
inline std::string encode_base32(uint64_t value) {
  ...
  std::string out;
  while (value > 0) {
    out.push_back(alphabet[value & 31]);
    value >>= 5;
  }
...
inline uint64_t hash_id(uint64_t id) {
...
inline std::tuple<uint8_t, uint8_t, uint8_t> oklch_to_srgb(float L, float C,
                                                          float h_rad) {
...
inline fmt::rgb color_for_id(uint64_t id) {
  uint64_t h = hash_id(id);
  float hue = (static_cast<float>(h % 36000) / 36000.0f) *
              2.0f * static_cast<float>(M_PI); // 0..2pi
  float L = 0.70f;
  float C = 0.08f;
...
inline fmt::text_style style_for_id(uint64_t id) {
  return fmt::fg(color_for_id(id));
}

inline std::string hashed_id_token(uint64_t id) { return encode_base32(hash_id(id)); }
