...
ev::Fields parse_json_fields(const json & arr)
{
    ev::Fields fields;
    for (const auto & field : arr) {
        if (field.is_string())
            ...
            fields.emplace_back(
                static_cast<uint64_t>(field.get<int64_t>()));
    }
    return fields;
}

/// Parse a JSON log entry and return the semantic event.
std::optional<Event> parse_json_event(const json & doc)
{
    ...
    if (action == "msg") {
        ...
        return ev::LogLine{
            .level =
                static_cast<nix::Verbosity>(doc["level"].get<int64_t>()),
