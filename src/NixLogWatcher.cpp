...
std::optional<std::string_view> activity_action(ActivityType type) {
  switch (type) {
...
std::optional<std::string> NixLogWatcher::format_activity_log_line(
...
NixLogWatcher::NixLogWatcher(bool quiet, UiMode ui_mode,
                             ...
                             std::atomic<bool> *stop_flag, double emit_delay_ms)
    : quiet_(quiet),
  ...
  if (ui_.enabled()) {
    render_thread_ =
        std::make_unique<std::thread>([this]() { this->render_loop(); });
  }
}
...
void NixLogWatcher::handle_stop_event(const StopEvent &e) {
  ...
  if (stopped_type == nix::actCopyPath ||
      stopped_type == nix::actFileTransfer ||
    ...
    if (stopped_info) {
      if (auto log_line =
              format_activity_log_line("<<<", fmt::terminal_color::red, e.id,
                                       parent, *stopped_info, label_fn)) {
        emit_log(*log_line);
        return;
      }
    } else {
...
void NixLogWatcher::rebuild_ui_state() {
  ...
  auto tag_for_type = [&](ActivityType type) {
    ...
    case nix::actFileTransfer:
      return std::string{""}; // No tag - it's obviously a download
    case nix::actQueryPathInfo:
  ...
  auto strip_store_hash = [](std::string_view name) {
    ...
    if (dash != std::string_view::npos && dash + 1 < name.size()) {
      std::string result{name.substr(dash + 1)};
      // Also strip .drv suffix if present
      if (result.ends_with(".drv")) {
  ...
  std::function<void(int64_t, int)> emit_node = [&](int64_t id, int depth) {
    ...
    // Filter out Realise and Builds activities - they're not useful to show
    // (Builds has bogus aggregate progress values, individual builds are shown)
    ...
    if (display_info && (display_info->type == nix::actBuild ||
                         display_info->type == nix::actBuildWaiting)) {
      if (display_info->store_path) {
        display_label = strip_store_hash(display_info->store_path->name());
      }
      if (!display_info->current_phase.empty()) {
        ...
        if (phase.size() > suffix.size() &&
            phase.compare(phase.size() - suffix.size(), suffix.size(),
                          suffix) == 0) {
    ...
    if (tag.empty()) {
      // No tag - don't add extra space
      name_part = fmt::format("{}{}", indent, display_label);
    } else {
      name_part = fmt::format("{}{} {}", indent, tag, display_label);
    }
    ...
    if (base_url && !base_url->empty()) {
      ...
      display = fmt::format("{:<{}} {:<{}}", name_part, name_col_width, url,
                            host_col_width);
    } else {
      display = name_part;
    }
    ...
    // For CopyPaths, show summary instead of progress bar
    if (display_info->type == nix::actCopyPaths && progress) {
      // Format as "Downloaded: X/Y" without progress bar
      if (progress->unit == ProgressUnit::Bytes) {
        display = fmt::format("{} Downloaded: {}/{}", display,
                              format_bytes(progress->done),
                              format_bytes(progress->expected));
      } else {
        display = fmt::format("{} {}/{}", display, progress->done,
                              progress->expected);
      }
      progress.reset(); // Clear progress to prevent bar rendering
    }
  ...
  // Sort activity lines: downloads (actFileTransfer) at the bottom
  // Stable sort preserves relative order within each group
  std::stable_sort(ui_state_.activity_lines.begin(),
                   ui_state_.activity_lines.end(),
                   [&](const UiActivityLine &a, const UiActivityLine &b) {
                     ...
                     bool is_dl_a = (info_a->type == nix::actFileTransfer);
                     bool is_dl_b = (info_b->type == nix::actFileTransfer);

                     // Non-downloads come before downloads
                     return !is_dl_a && is_dl_b;
                   });
}

void NixLogWatcher::refresh_ui() {
  std::lock_guard<std::mutex> lock(state_mutex_);
  rebuild_ui_state();
  ui_.hud().present(ui_state_);
}

void NixLogWatcher::render_loop() {
...
void NixLogWatcher::process_playback_file(const std::string &path,
                                          double speedup) {
  NixLogPlayer player([this](const std::string &line) { process_line(line); },
                      stop_flag_);
