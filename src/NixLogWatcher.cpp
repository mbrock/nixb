  ...
  if (e.type == nix::resSetExpected) {
    ...
    auto has_activity = [&](ActivityType t) {
      return std::any_of(activities_.begin(), activities_.end(),
                         [&](const auto &kv) { return kv.second.type == t; });
    };
...
void NixLogWatcher::record_line(const std::string &line) {
...
void NixLogWatcher::note_transfer_start(int64_t id) {
...
void NixLogWatcher::note_transfer_stop(int64_t id) {
  ...
  for (const auto &kv : activities_) {
    ...
    if (info.type == nix::actBuild || info.type == nix::actBuildWaiting) {
      ...
      ui_state_.active_builds.push_back(
          SingleBuildState{kv.first, std::move(label), std::move(status)});
    }
  }
...
void NixLogWatcher::rebuild_active_transfers() {
