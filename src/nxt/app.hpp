...
public:
  ...
  [[nodiscard]] nxb::io_scheduler &scheduler() noexcept { return *scheduler_; }

  /// Access the glyph table.
  [[nodiscard]] GlyphTable &glyphs() noexcept { return glyphs_; }
  ...
  template <typename... Tasks> auto run(Tasks &&...tasks) {
    auto tg = nxb::task_group(this->scheduler(), std::forward<Tasks>(tasks)...);
    return tg.run_all();
  }
  ...
  template <typename Layout> void render(const Layout &layout) {
    ...
    render_impl(
        [&layout](RasterView &view, Size size) { layout.render(view, size); });
  }
  ...
  nxb::queue<TermSize> &resize_channel() noexcept { return resize_queue_; }

  /// Event signaled when damage occurs.
  nxb::event &damage_event() noexcept { return damage_event_; }
