...
namespace nxb
{
  ...
  {
    fmt::print (os, "rgba8({:02x},{:02x},{:02x},{:02x})", c.r (), c.g (),
                c.b (), c.a ());
    return os;
  }
    ...
    while (i < text.size () && col < cols)
      {
        ...
        const GlyphTable::GlyphId gid
            = glyph_table_->intern (glyph_bytes);
  ...
  Raster::Raster (const width_t width, const height_t height,
      ...
      : width_ (width), height_ (height),
        ...
        fgs_storage_ (cols_from (width) * rows_from (height),
                      DEFAULT_COLOR),
        bgs_storage_ (cols_from (width) * rows_from (height),
                      DEFAULT_COLOR),
        ems_storage_ (cols_from (width) * rows_from (height),
                      DEFAULT_EMPHASIS),
        glyph_table_ (&glyphs)
  {
  }
  ...
  Raster::view () noexcept
  {
    ...
    glyph_view_t glyphs (glyphs_storage_.data (),
    ...
    emphasis_view_t ems (ems_storage_.data (),
                         mdspan_extents{ rows, cols });
