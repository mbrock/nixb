...
namespace nxb::tui
{
  ...
  // Emphasis shortcuts
  inline constexpr Style bold{ DEFAULT_COLOR, DEFAULT_COLOR,
                               Emphasis::bold };
  inline constexpr Style faint{ DEFAULT_COLOR, DEFAULT_COLOR,
                                Emphasis::faint };
  ...
  text (std::string s, Style style)
  {
    auto w = utf8_width (s);
    return leaf (
        WidthHint::fixed (w), HeightHint::fixed (1 * ln),
        [=] (RasterView &r, Size)
  ...
  {
    return leaf (
        WidthHint::grow (), HeightHint::fixed (1 * ln),
  ...
  bar_string (percent_t pct, width_t width)
  {
    ...
    auto fraction
        = std::clamp (pct.numerical_value_in (percent), 0.0, 100.0)
          / 100.0;
    double fill = fraction * w;
    std::size_t full = static_cast<std::size_t> (fill);
    std::size_t partial
        = static_cast<std::size_t> ((fill - full) * 8 + 0.5);
                 ...
                 [=] (RasterView &r, Size)
                   {
                     ...
                     r.write_text (Pos::origin (),
                                   bar_string (pct, r.width ()));
                   });
  }
  ...
  template <Layout... Children> struct Row
  {
    ...
    height_hint () const
    {
      ...
      std::apply (
          ...
          children);
      return HeightHint::fixed (max_min.numerical_value_in (ln) > 0
                                    ? max_min
                                    : height_t{ 1 * ln });
    }
  ...
  private:
    ...
    flex_distribute (height_t total,
                     const std::array<HeightHint, N> &hints)
    {
