...
//   auto drv_path = ctx.store()->parseStorePath("/nix/store/...-foo.drv");
//   auto deps = nxb::get_derivation_deps(ctx, drv_path);
//   for (const auto& [drv, inputs] : deps) {
//     fmt::print("{} depends on {} derivations\n", drv.name(), inputs.size());
//   }
...
inline std::string
format_phase_name (std::string_view phase)
{
  constexpr std::string_view suffix = "Phase";
  std::string result{ phase };
  if (result.size () > suffix.size ()
      && result.compare (result.size () - suffix.size (), suffix.size (),
                         suffix)
             == 0)
    {
...
public:
  ...
  store ()
  {
    return nix::ref<nix::Store> (store_);
  }
  ...
  eval_state ()
  {
    if (!eval_state_)
      {
        bool read_only = true;
        auto eval_settings = nix::EvalSettings{ read_only };
        auto fetch_settings = nix::fetchers::Settings{};
        auto lookup_path = nix::LookupPath{};

        eval_state_ = std::make_shared<nix::EvalState> (
            lookup_path, store (), fetch_settings, eval_settings);
      }
  ...
  try
    {
      ...
      for (const auto &[output_name, output] : drv.outputs)
        if (auto path
            = output.path (*ctx.store_ptr (), drv.name, output_name))
...
inline std::optional<nix::StorePath>
get_build_drv (NixContext &ctx, const nix::StorePath &path)
{
  try
    {
...
resolve_installable (NixContext &ctx, const std::string &installable)
{
  ...
  auto drv_set
      = nix::Installable::toDerivations (store_ref, installables, true);
