...
inline std::string format_phase_name(std::string_view phase)
{
    ...
    if (result.size() > suffix.size()
        && result.compare(
               result.size() - suffix.size(), suffix.size(), suffix)
               == 0) {
...
public:
    ...
    std::shared_ptr<nix::Store> store_ptr()
    {
        return store_;
    }
...
struct DerivationInfo
{
    ...
    DerivationInfo(nix::StorePath p)
        : path(std::move(p))
    {
    }
};
...
build_dependency_graph(
    NixContext & ctx, const std::vector<nix::StorePath> & drv_paths)
{
