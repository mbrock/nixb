...
suite vterm_tests = [] suite vterm_tests = [] {
  ...
  "basic-ansi: ANSI escape sequences render correctly"_test = [] {
    ...
    expect (eq (trim_end (term.get_row_text (0)), "HELLO"));
    expect (eq (trim_end (term.get_row_text (1)), "WORLD!!"));
    expect (eq (trim_end (term.get_row_text (2)), ""));
    expect (eq (trim_end (term.get_row_text (3)), ""));
  };
  ...
  "vterm: wide characters (CJK)"_test = [] {
    ...
    expect (snap.any_of ([] (const Cell &cell) { return cell.width == 2; }));
  ...
  "vterm: cursor movement"_test = [] {
    ...
    expect (eq (term.get_row_text (0)[0], 'C'));
    expect (eq (term.get_row_text (4)[9], 'B'));
  };
  ...
  "vterm: screen clearing"_test = [] {
    ...
    expect (snap.all_of ([] (const Cell &cell) {
  ...
  "vterm: get screen text"_test = [] {
    ...
    // Get individual rows
    expect (eq (term.get_row_text (0).substr (0, 3), "AAA"));
    expect (eq (term.get_row_text (1).substr (0, 3), "BBB"));
    expect (eq (term.get_row_text (2).substr (0, 3), "CCC"));
    ...
    for (int row = 0; row < 5; row++)
      {
        ...
        expect (eq (expected_text, actual_text))
