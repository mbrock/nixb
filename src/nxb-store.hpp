...
struct TrivialStore : virtual nix::Store {
  ...
  struct StoredPath {
    ...
    StoredPath(nix::UnkeyedValidPathInfo i,
               std::shared_ptr<nix::MemorySourceAccessor> c)
        : info(std::move(i)), contents(std::move(c)) {}
  };
  ...
  TrivialStore(nix::ref<const Config> config, ui::UIRuntime& runtime)
      : nix::Store{*config}, config(config), runtime(runtime) {}

  void print(std::string_view msg) { runtime.println(std::string(msg)); }
  ...
  void queryPathInfoUncached(
    ...
    if (it != storage.end()) {
      callback(std::make_shared<nix::ValidPathInfo>(nix::StorePath{path},
                                                    it->second.info));
    } else {
      callback(nullptr);
    }
  ...
  std::optional<nix::StorePath> queryPathFromHashPart(
    ...
    for (const auto& [path, _] : storage) {
      if (path.hashPart() == hashPart) return path;
    }
    return std::nullopt;
  }
  ...
  void addToStore(const nix::ValidPathInfo& info, nix::Source& source,
    ...
    print(fmt::format("  stored {} ({} refs)", info.path.name(),
  ...
  nix::StorePath addToStoreFromDump(nix::Source& source, std::string_view name,
    ...
    print(fmt::format("addToStoreFromDump: {} ({} refs)", name, refs.size()));
      ...
      switch (dumpMethod) {
        ...
        case nix::FileSerialisationMethod::Flat:
          ...
          sink.createRegularFile(nix::CanonPath::root,
                                 [&](auto& s) { source.drainInto(s); });
          break;
      }
    }

    // Compute hash
    auto hash = nix::hashPath({contents, nix::CanonPath::root},
                              hashMethod.getFileIngestionMethod(), hashAlgo)
                    .first;
    auto narHash = nix::hashPath({contents, nix::CanonPath::root},
    ...
    auto info = nix::ValidPathInfo::makeFromCA(
        *this, name,
        nix::ContentAddressWithReferences::fromParts(
            hashMethod, std::move(hash), {.others = refs, .self = false}),
  ...
  void narFromPath(const nix::StorePath& path, nix::Sink& sink) override {
    ...
    auto it = storage.find(path);
    if (it == storage.end())
      throw nix::Error("path '%s' is not valid", printStorePath(path));
  ...
  std::shared_ptr<nix::SourceAccessor> getFSAccessor(const nix::StorePath& path,
    ...
    std::lock_guard lock(storage_mutex);
    auto it = storage.find(path);
    if (it != storage.end()) return it->second.contents;
    return nullptr;
  }
...
// Note: openStore() requires runtime, so we implement it via makeTrivialStore
inline nix::ref<nix::Store> TrivialStoreConfig::openStore() const {
  throw nix::Error("TrivialStore requires UIRuntime - use makeTrivialStore()");
}

// Helper to create a TrivialStore with UIRuntime reference
inline std::shared_ptr<TrivialStore> makeTrivialStore(ui::UIRuntime& runtime) {
  auto config =
