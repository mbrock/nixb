...
inline constexpr struct page final
    : named_unit<"page", mag<4096> * iec::bit>
{
} page;

// Variant to hold different quantity types
using stat_value = std::variant<
    decltype(1.0 * B),
    decltype(1.0 * page),
    decltype(1.0 * us),
    decltype(1.0 * percent),
...
constexpr std::array memory_stat_fields = {
    ...
    field_info{
        "file", "cached file data (total)", field_info::unit_type::bytes},
    field_info{"kernel", "kernel data", field_info::unit_type::bytes},
    field_info{
        "kernel_stack", "kernel stack data", field_info::unit_type::bytes},
    field_info{
        "pagetables", "page table data", field_info::unit_type::bytes},
    field_info{
        "slab", "cached kernel objects", field_info::unit_type::bytes},
    field_info{
        "slab_reclaimable",
        "cached kernel objects (reclaimable)",
        field_info::unit_type::bytes},
    field_info{
        "slab_unreclaimable",
        "cached kernel objects (unreclaimable)",
        field_info::unit_type::bytes},
    field_info{"sock", "socket buffer data", field_info::unit_type::bytes},
    field_info{
        "vmalloc",
        "virtually allocated data",
        field_info::unit_type::bytes},
    field_info{"shmem", "shared memory data", field_info::unit_type::bytes},
    field_info{
        "file_mapped", "mapped file data", field_info::unit_type::pages},
    field_info{
        "file_dirty", "dirty file data", field_info::unit_type::pages},
    field_info{
        "file_writeback",
        "file data (writeback queue)",
        field_info::unit_type::pages},
    field_info{
        "swapcached", "cached swap data", field_info::unit_type::bytes},
    field_info{
        "anon_thp",
        "non-file data (transparent huge pages)",
        field_info::unit_type::bytes},
    field_info{
        "file_thp",
        "file data (transparent huge pages)",
        field_info::unit_type::bytes},
    field_info{
        "inactive_anon",
        "non-file data (inactive)",
        field_info::unit_type::bytes},
    field_info{
        "active_anon",
        "non-file data (active)",
        field_info::unit_type::bytes},
    field_info{
        "inactive_file",
        "cached file data (inactive)",
        field_info::unit_type::bytes},
    field_info{
        "active_file",
        "cached file data (active)",
        field_info::unit_type::bytes},
    field_info{
        "unevictable", "protected data", field_info::unit_type::bytes},
    field_info{
        "pgfault",
        "memory access fault events",
        field_info::unit_type::count},
    field_info{
        "pgmajfault",
        "memory access fault events (disk-backed)",
        field_info::unit_type::count},
    field_info{
    ...
    field_info{
        ...
        "working set node reclaim events",
        field_info::unit_type::count},
    field_info{
        "pgscan",
        "memory reclaim scan events",
        field_info::unit_type::count},
    field_info{
        "pgsteal",
        "memory reclaim eviction events",
        field_info::unit_type::count},
    field_info{
        "pgscan_kswapd",
        "memory reclaim scan events (kswapd)",
        field_info::unit_type::count},
    field_info{
        "pgscan_direct",
        "memory reclaim scan events (direct)",
        field_info::unit_type::count},
    field_info{
        "pgsteal_kswapd",
        "memory reclaim eviction events (kswapd)",
        field_info::unit_type::count},
    field_info{
        "pgsteal_direct",
        "memory reclaim eviction events (direct)",
        field_info::unit_type::count},
    field_info{
        "pgactivate",
        "cache promotion events",
        field_info::unit_type::count},
    field_info{
        "pgdeactivate",
        "cache demotion events",
        field_info::unit_type::count},
    field_info{
        "pglazyfree",
        "lazy free events (scheduled)",
        field_info::unit_type::count},
    field_info{
        "pglazyfreed",
        "lazy free events (completed)",
        field_info::unit_type::count},
    field_info{
        "thp_fault_alloc",
...
constexpr std::array cpu_stat_fields = {
    ...
    field_info{
        "user_usec", "user mode time", field_info::unit_type::microseconds},
    field_info{
        "system_usec",
        "kernel mode time",
        field_info::unit_type::microseconds},
    field_info{
        "nr_periods", "enforcement periods", field_info::unit_type::count},
    field_info{
        "nr_throttled", "throttled periods", field_info::unit_type::count},
    field_info{
        "throttled_usec",
...
stat_value parse_value(std::uint64_t raw, field_info::unit_type unit)
{
    switch (unit) {
...
std::string format_value(const stat_value & value)
{
    return std::visit(
        [](auto && val) -> std::string {
            ...
            if constexpr (std::is_same_v<T, std::uint64_t>) {
                ...
                if (val_numeric >= 3600000000UL)
                    return fmt::format("{::N[.2f]}", val.in(h));
                else if (val_numeric >= 60000000UL)
                    return fmt::format("{::N[.2f]}", val.in(min));
                else if (val_numeric >= 1000000UL)
                    return fmt::format("{::N[.2f]}", val.in(s));
                else if (val_numeric >= 1000UL)
...
parse_memory_stat(std::string_view contents)
{
...
std::map<std::string, stat_value> parse_cpu_stat(std::string_view contents)
{
    std::map<std::string, stat_value> stats;
    std::istringstream file{std::string(contents)};
    std::string line;
...
std::vector<io_device_stats> parse_io_stat(std::string_view contents)
{
...
void print_field_rows(
    std::span<const field_info> fields,
    const std::map<std::string, stat_value> & stats,
    unsigned indent)
{
    ...
    std::string indent_padding(indent, ' ');
    for (const auto & [label, value] : rows) {
        print(
            "{}{: <{}}  {}\n",
            indent_padding,
...
void scan()
{
    ...
    if (auto content = read_file(cgroup_path / "cpu.stat");
        !content.empty()) {
    ...
    if (auto content = read_file(cgroup_path / "io.stat");
        !content.empty()) {
        ...
        for (const auto & device : devices) {
            print(
                ...
                fmt::styled(
                    device.device_id,
                    fmt::fg(fmt::color::sky_blue) | fmt::emphasis::bold));
