...
struct read_file_impl : __stoppable_op_base<Receiver>
{
  ...
  read_file_impl (__context &ctx,
                  std::filesystem::path &&path,
                  std::size_t buffer_bytes,
                  Receiver &&receiver)
      : __stoppable_op_base<Receiver>{ ctx, static_cast<Receiver &&> (receiver) },
        path_{ std::move (path) },
        buffer_ (buffer_bytes, '\0')
  {
    int raw_fd = ::open (path_.c_str (), O_RDONLY | O_CLOEXEC);
    if (raw_fd < 0)
      {
        open_failed_ = true;
        open_error_ = std::make_exception_ptr (
...
template <class Receiver>
using read_file_operation = __stoppable_task_facade_t<read_file_impl<Receiver>>;
...
public:
  ...
  friend auto
  tag_invoke (stdexec::connect_t,
              io_uring_file_sender &&self,
...
{
  return detail::io_uring_file_sender (scheduler,
                                       std::move (path),
                                       buffer_bytes);
}
                 ...
                 Printer printer)
{
  ...
  return async_read_file (scheduler, std::move (path))
...
struct io_uring_runtime
{
  ...
  io_uring_runtime ()
      : context (),
        worker ([this] { context.run_until_stopped (); })
  {
  }
