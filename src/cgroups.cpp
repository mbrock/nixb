...
inline constexpr struct page final : named_unit<"page", mag<4096> * iec::bit> {
} page;
...
struct field_info {
  ...
  enum class unit_type { bytes, pages, microseconds, percent, count } unit;
};
...
constexpr std::array memory_stat_fields = {
    ...
    field_info{"file_mapped", "mapped file data", field_info::unit_type::pages},
    ...
    field_info{"active_file", "cached file data (active)",
               field_info::unit_type::bytes},
    field_info{"unevictable", "protected data", field_info::unit_type::bytes},
    ...
    field_info{"workingset_refault_anon",
               ...
               field_info::unit_type::count},
    field_info{"workingset_refault_file", "working set refault events (file)",
               field_info::unit_type::count},
...
std::map<std::string, stat_value> parse_memory_stat(std::string_view contents) {
...
void print_field_rows(std::span<const field_info> fields,
  ...
  for (const auto &field : fields) {
    if (auto it = stats.find(std::string(field.name)); it != stats.end()) {
  ...
  for (const auto &[label, value] : rows) {
    print("{}{: <{}}  {}\n", indent_padding,
          fmt::styled(label, fmt::fg(fmt::color::lawn_green)), max_label,
          fmt::styled(value,
...
void scan() {
  ...
  for (const auto &filename : other_files) {
    ...
    if (content.is_open()) {
      print("{}\n",
            fmt::styled(filename, fmt::fg(fmt::color::light_steel_blue)));
      std::string line;
