...
std::string
get_string_field (const Fields &fields, const size_t idx)
{
  if (idx >= fields.size ())
    return {};
...
int64_t
get_int_field (const Fields &fields, const size_t idx)
{
  if (idx >= fields.size ())
    return 0;
...
activity::Kind
parse_activity_kind (const nix::ActivityType type, const std::string &text,
                     const Fields &fields)
{
...
NixLogAdapter::NixLogAdapter (coro::queue<Event> &queue) : queue_ (queue) {}
...
void
NixLogAdapter::logEI (const nix::ErrorInfo &ei)
{
  coro::sync_wait (queue_.push (ev::Error{ .info = ei }));
}

void
NixLogAdapter::startActivity (const nix::ActivityId act,
...
NixLogAdapter::stopActivity (const nix::ActivityId act)
{
  coro::sync_wait (queue_.push (
      ev::ActivityFinished{ .id = ev::ActivityId{ static_cast<int64_t> (act) } }));
}

void
NixLogAdapter::result (const nix::ActivityId act, const nix::ResultType type,
                       const Fields &fields)
{
