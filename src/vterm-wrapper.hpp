#pragma once
// Minimal C++ wrapper for libvterm - render ANSI to virtual terminal and read
// back screen state. Usage: Terminal term(24, 80);
...
struct Color
{
  ...
  [[nodiscard]] bool
  is_rgb () const noexcept
  {
    return VTERM_COLOR_IS_RGB (&c);
  }

  [[nodiscard]] bool
  is_indexed () const noexcept
  {
    return VTERM_COLOR_IS_INDEXED (&c);
  }

  [[nodiscard]] bool
  is_default_fg () const noexcept
  {
    return VTERM_COLOR_IS_DEFAULT_FG (&c);
  }

  [[nodiscard]] bool
  is_default_bg () const noexcept
  {
...
struct Cell
{
  ...
  Cell () noexcept
      : width (1), bold (false), italic (false), underline (false),
  ...
  static Cell
  from_vterm (const VTermScreenCell &cell) noexcept
  {
    Cell c;
...
public:
  ...
  struct ScreenSnapshot
  {
    ...
    using mdspan_extents = std::experimental::extents<int, std::dynamic_extent,
                                                      std::dynamic_extent>;
